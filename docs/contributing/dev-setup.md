...
| [PowerShell 7](https://learn.microsoft.com/en-us/powershell/scripting/install/installing-powershell-on-windows) | 7.x | Running build and deploy scripts (`pwsh`) |
...
winget install -e --id Microsoft.PowerShell @autoaccept
...
4. **Render workflow SVGs** - `pwsh docs/build.ps1`

   Workflow SVGs are not committed to the repository. `build.ps1` builds the NuGet packages, bootstraps the Bonsai render environment, and writes SVGs into `docs/assets/workflows/` and `docs/workflows/`. This step is required before serving the docs locally, otherwise workflow diagrams will be missing.

5. **Preview the docs** - `dotnet docfx docs/docfx.json --serve`
